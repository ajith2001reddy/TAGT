...
export default function AdminDashboard() {
    ...
    const fetchRequests = useCallback(async () => {
        try {
            ...
            const active = safeData.filter(
                (r) => r.status !== "resolved" && r.workflowStatus !== "Done"
            );
    ...
    const chartData = [
        ...
        { status: "Resolved", count: allRequests.filter(r => r.status === "resolved").length },
    ];
        ...
        >
            ...
            {statsLoading || revenueLoading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                    <StatCard title="Total Residents" value={stats.totalResidents || 0} accent="blue" />
                    <StatCard title="Pending Requests" value={stats.pendingRequests || 0} accent="yellow" />
                    <StatCard title="Total Revenue" value={`$${totalRevenue}`} accent="green" />
                    <StatCard title="Outstanding Balance" value={`$${outstandingBalance}`} accent="red" />
                </div>
            )}
