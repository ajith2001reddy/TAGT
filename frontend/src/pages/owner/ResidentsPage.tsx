...
export default function OwnerResidentsPage() {
  ...
  const load = () => api.get('/v2/residents').then(r => setItems(r.data.data || []))
  useEffect(() => { load() }, [])
  const add = async (e: React.FormEvent) => { e.preventDefault(); await api.post('/v2/residents', form); setForm({name:'',email:'',password:''}); load() }
