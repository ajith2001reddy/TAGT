...
export default function OwnerRoomsPage() {
  ...
  const load = () => api.get('/v2/owner/rooms').then(r => setRooms(r.data.data || []))
  useEffect(() => { load() }, [])
  const add = async (e: React.FormEvent) => { e.preventDefault(); await api.post('/v2/owner/rooms', form); setForm({ roomNumber:'', totalBeds:1, rentAmount:0 }); load() }
